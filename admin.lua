--[[
    ADMIN HUB PREMIUM EDITION (SERVER-AUTH VERSION)
    Toggle: Left Control
    Status: Redesigned & Draggable
]]

local player = game.Players.LocalPlayer
local mouse = player:GetMouse()
local event = game:GetService("ReplicatedStorage"):WaitForChild("AdminRemotes"):WaitForChild("CommandEvent")
...
hub.CanvasSize = UDim2.new(0, 0, 0, 480) -- Aumentado para acomodar novos campos
...
-- LÓGICA DE EVENTOS (SERVER-SIDE AUTH)
...
spawnToolBtn.MouseButton1Click:Connect(function()
    ...
    spawnTool.Activated:Connect(function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            -- Calcula a posição baseada na distância configurada no UI
            local spawnPos = char.HumanoidRootPart.CFrame * CFrame.new(0, 0, -spawnDistance)
            -- Envia para o servidor (Ajuste o servidor para aceitar posição se necessário, 
            -- ou o servidor pode apenas rodar o spawn no lugar do player)
